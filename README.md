# Clash Verge Rev 多电脑配置一键同步与恢复工具
...
* **执行逻辑**：
  ...
  * **还原模式 (`-Restore`)**：若传入此开关参数，脚本将自动检索当前用户 AppData 目录下的 `verge.yaml.bak` and `Merge.yaml.bak` 备份文件，并将其强制覆盖还原，随后强杀重启客户端应用配置，实现一键无感退回。
  * **部署模式 (常规运行)**：
...
* **执行逻辑**：
  * 自动切换当前工作目录，调用 PowerShell 以管理员权限及 `-ExecutionPolicy Bypass` 策略运行 `Deploy-ClashVerge.ps1` 核心脚本（还原批处理会自动追加 `-Restore` 参数），免去手动调整系统执行策略的繁琐步骤。
...
````markdown
我需要你拉取这个项目，阅读项目说明，帮我完成 clash verge 的配置文件更新。具体执行规范如下：

1. **自动定位并拉取项目**：
   * 请直接使用 Git 命令将本项目克隆至本地的工作区目录中。
     仓库地址：https://github.com/timothydeng233-web/DDDD_clash_verge_config_script.git

2. **独立安全审计 (Security Audit)**：
   * 请立即阅读并核对克隆下来的 `Deploy-ClashVerge.ps1` 和 `ConfigBackup/` 目录下的 `verge.yaml`、`Merge.yaml` 物理文件内容。
   * **安全审计红线指标**：
     * **隐私泄露评估**：确保脚本与配置文件中不含有任何硬编码的真实订阅链接（`url: "https://..."`）、不含有任何 API 密钥、以及任何属于原作者个人的隐私物理路径。
...
3. **智能决策与自动化部署 (Deployment Execution)**：
   * 如果上述安全审计红线指标**全部通过**（风险属于极其合理的系统优化范畴，且具备完善的回滚恢复手段）：
     * 请使用您的运行命令行工具，以**管理员身份**启动并执行目录下的 `Deploy-ClashVerge.ps1` 脚本（或直接双击拉起 `双击一键统一部署.bat`）。
